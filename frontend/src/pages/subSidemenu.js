...
import { ServiceListResults } from '../components/subSideMenu/subSideMenu-list-results';
import { ServiceListToolbar } from '../components/subSideMenu/subSideMenu-list-toolbar';
import { ServiceModal } from '../components/subSideMenu/modal';
...
const Page = () => {
  ...
  const [sidemenudata, setsidemenu] = useState();
  ...
  const fetchData = () => {
    ...
    const url = `${API_BASE_URL}subsidemenu/list_subsidemenu`;
    ApiService.get(url, headers)
      ...
      const newurl = `${API_BASE_URL}sidemenu/list_sidemenu`;
    ApiService.get(newurl, headers)
      .then(res => {
        const { data } = res.data;
        setsidemenu(data);
      })
  ...
  const onDelete = (serviceId) => {
    ...
    const url = `${API_BASE_URL}subsidemenu/delete_subsidemenu/${serviceId}`;
    axios.delete(url, {
      headers: headers,
    }).then(res => {
        ...
        <Container maxWidth={false}>
          ...
          >
            ...
            <DialogContent>
              ...
              >
                <ServiceModal service={service} sidemenu = {sidemenudata} onClose={() => closeModal()} />
              </Box>
            </DialogContent>
            <DialogActions>

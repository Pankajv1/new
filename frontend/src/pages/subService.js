...
import { ServiceListResults } from '../components/subService/subservice-list-results';
import { ServiceListToolbar } from '../components/subService/subservice-list-toolbar';
import { ServiceModal } from '../components/subService/modal';
...
const Page = () => {
  ...
  const [servicename, setDataservice] = useState();
  ...
  const fetchData = () => {
    ...
    const url = `${API_BASE_URL}subservice/list_subservice`;
    ApiService.get(url, headers)
      ...
      const urlnew = `${API_BASE_URL}service/list_service`;
      ApiService.get(urlnew, headers)
        .then(res => {
          const { data } = res.data;
  ...
  const onDelete = (serviceId) => {
    ...
    const url = `${API_BASE_URL}subservice/delete_subservice/${serviceId}`;
    axios.delete(url, {
      headers: headers,
    }).then(res => {
        ...
        <Container maxWidth={false}>
          ...
          >
            ...
            <DialogContent>
              ...
              >
                <ServiceModal service={service} sevicename={servicename} onClose={() => closeModal()} />
              </Box>
            </DialogContent>
            <DialogActions>

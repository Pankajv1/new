...
import { ServiceListResults } from '../components/customer/user-list-results';
import { ServiceListToolbar } from '../components/customer/user-list-toolbar';
import { ServiceModal } from '../components/customer/modal';
...
const Page = () => {
  ...
  const fetchData = () => {
    ...
    const url = `${API_BASE_URL}user/all_user`;
  ...
  const onDelete = (serviceId) => {
    ...
    const url = `${API_BASE_URL}user/deleteuser/${serviceId}`;
    axios.delete(url, {
      headers: headers,
    }).then(res => {
